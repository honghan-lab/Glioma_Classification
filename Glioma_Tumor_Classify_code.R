...
limma_filter = function(x, y, nfilter) {
  ...
  all_top_genes = unique(unlist(lapply(1:ncol(contrast_matrix), function(i){top = head(limma::topTable(fit2, coef = i, number = ncol(x), sort.by = "p", adjust.method="BH")  %>% .[order(.$adj.P.Val),],nfilter);rownames(top)})))
...
cvfit_glm_lasso=nestcv.glmnet(y = y, x = x, family = "multinomial", cv.cores = 10, alphaSet = 1, n_outer_folds = 5, n_inner_folds = 5, finalCV = T, filterFUN = limma_filter, filter_options = list(nfilter=100), verbose = T)
cvfit_glm_ridge=nestcv.glmnet(y = y, x = x, family = "multinomial", cv.cores = 10, alphaSet = 0, n_outer_folds = 5, n_inner_folds = 5, finalCV = T, filterFUN = limma_filter, filter_options = list(nfilter=100), verbose = T)
cvfit_glm_elasticnet=nestcv.glmnet(y = y, x = x, family = "multinomial", cv.cores = 10, alphaSet = 0.5, n_outer_folds = 5, n_inner_folds = 5, finalCV = T, filterFUN = limma_filter, filter_options = list(nfilter=100), verbose = T)
cvfit_rf=nestcv.train(y = y, x = x, method = "rf",n_outer_folds = 5,n_inner_folds = 5, filterFUN = limma_filter, filter_options = list(nfilter=100),finalCV = T,tuneGrid=expand.grid(mtry=sqrt(ncol(x))),cv.cores = 10,savePredictions = "final")
cvfit_xgb=nestcv.train(y = y, x = x, method = "xgbDART",n_outer_folds = 5,n_inner_folds = 5, filterFUN = limma_filter, filter_options = list(nfilter=100),finalCV = T,tuneGrid=expand.grid(nrounds = 10, max_depth = 2, eta = 0.30, rate_drop = 0.10, skip_drop = 0.10, colsample_bytree = 0.90, min_child_weight = 2, subsample = 0.75, gamma = 0.10),cv.cores = 10,savePredictions = "final")
...
tcga_glm_lasso=data.frame(predict(cvfit_glm_lasso,newdata = as.matrix(t(cve_gene_gbmlgg_tcga[,colnames(cve_gene_gbmlgg_tcga) %in% rownames(factors_gbmlgg_tcga)])),type="response"))
colnames(tcga_glm_lasso)=gsub("\\..*", "", colnames(tcga_glm_lasso))
tcga_glm_ridge=data.frame(predict(cvfit_glm_ridge,newdata = as.matrix(t(cve_gene_gbmlgg_tcga[,colnames(cve_gene_gbmlgg_tcga) %in% rownames(factors_gbmlgg_tcga)])),type="response"))
colnames(tcga_glm_ridge)=gsub("\\..*", "", colnames(tcga_glm_ridge))
tcga_glm_elasticnet=data.frame(predict(cvfit_glm_elasticnet,newdata = as.matrix(t(cve_gene_gbmlgg_tcga[,colnames(cve_gene_gbmlgg_tcga) %in% rownames(factors_gbmlgg_tcga)])),type="response"))
colnames(tcga_glm_elasticnet)=gsub("\\..*", "", colnames(tcga_glm_elasticnet))
tcga_rf=data.frame(predict(cvfit_rf,newdata = as.matrix(t(cve_gene_gbmlgg_tcga[,colnames(cve_gene_gbmlgg_tcga) %in% rownames(factors_gbmlgg_tcga)])),type="prob"))
tcga_xgb=data.frame(predict(cvfit_xgb,newdata = as.matrix(t(cve_gene_gbmlgg_tcga[,colnames(cve_gene_gbmlgg_tcga) %in% rownames(factors_gbmlgg_tcga)])),type="prob"))

cgga_glm_lasso=data.frame(predict(cvfit_glm_lasso,newdata = as.matrix(t(cve_gene_gbmlgg_cgga[,colnames(cve_gene_gbmlgg_cgga) %in% rownames(factors_gbmlgg_cgga)])),type="response"))
colnames(cgga_glm_lasso)=gsub("\\..*", "", colnames(cgga_glm_lasso))
cgga_glm_ridge=data.frame(predict(cvfit_glm_ridge,newdata = as.matrix(t(cve_gene_gbmlgg_cgga[,colnames(cve_gene_gbmlgg_cgga) %in% rownames(factors_gbmlgg_cgga)])),type="response"))
colnames(cgga_glm_ridge)=gsub("\\..*", "", colnames(cgga_glm_ridge))
cgga_glm_elasticnet=data.frame(predict(cvfit_glm_elasticnet,newdata = as.matrix(t(cve_gene_gbmlgg_cgga[,colnames(cve_gene_gbmlgg_cgga) %in% rownames(factors_gbmlgg_cgga)])),type="response"))
colnames(cgga_glm_elasticnet)=gsub("\\..*", "", colnames(cgga_glm_elasticnet))
cgga_rf=data.frame(predict(cvfit_rf,newdata = as.matrix(t(cve_gene_gbmlgg_cgga[,colnames(cve_gene_gbmlgg_cgga) %in% rownames(factors_gbmlgg_cgga)])),type="prob"))
cgga_xgb=data.frame(predict(cvfit_xgb,newdata = as.matrix(t(cve_gene_gbmlgg_cgga[,colnames(cve_gene_gbmlgg_cgga) %in% rownames(factors_gbmlgg_cgga)])),type="prob"))

glass_glm_lasso=data.frame(predict(cvfit_glm_lasso,newdata = as.matrix(t(cve_gene_gbmlgg_glass[,colnames(cve_gene_gbmlgg_glass) %in% rownames(factors_gbmlgg_glass)])),type="response"))
colnames(glass_glm_lasso)=gsub("\\..*", "", colnames(glass_glm_lasso))
glass_glm_ridge=data.frame(predict(cvfit_glm_ridge,newdata = as.matrix(t(cve_gene_gbmlgg_glass[,colnames(cve_gene_gbmlgg_glass) %in% rownames(factors_gbmlgg_glass)])),type="response"))
colnames(glass_glm_ridge)=gsub("\\..*", "", colnames(glass_glm_ridge))
glass_glm_elasticnet=data.frame(predict(cvfit_glm_elasticnet,newdata = as.matrix(t(cve_gene_gbmlgg_glass[,colnames(cve_gene_gbmlgg_glass) %in% rownames(factors_gbmlgg_glass)])),type="response"))
colnames(glass_glm_elasticnet)=gsub("\\..*", "", colnames(glass_glm_elasticnet))
glass_rf=data.frame(predict(cvfit_rf,newdata = as.matrix(t(cve_gene_gbmlgg_glass[,colnames(cve_gene_gbmlgg_glass) %in% rownames(factors_gbmlgg_glass)])),type="prob"))
glass_xgb=data.frame(predict(cvfit_xgb,newdata = as.matrix(t(cve_gene_gbmlgg_glass[,colnames(cve_gene_gbmlgg_glass) %in% rownames(factors_gbmlgg_glass)])),type="prob"))


temp_pr_classify_glm_lasso=lapply(1:6, function(i){pr.curve(scores.class0 = na.omit(tcga_glm_lasso[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A" & factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis==(c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i],i]), scores.class1 = na.omit(tcga_glm_lasso[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A" & factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!=(c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i],i]), curve = TRUE )})
plot_pr_classify_glm_lasso = bind_rows(lapply(1:6, function(i){curve = temp_pr_classify_glm_lasso[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i])}))
temp_pr_classify_glm_ridge=lapply(1:6, function(i){pr.curve(scores.class0 = na.omit(tcga_glm_ridge[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A" & factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis==(c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i],i]), scores.class1 = na.omit(tcga_glm_ridge[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A" & factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!=(c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i],i]), curve = TRUE )})
plot_pr_classify_glm_ridge = bind_rows(lapply(1:6, function(i){curve = temp_pr_classify_glm_ridge[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i])}))
temp_pr_classify_glm_elasticnet=lapply(1:6, function(i){pr.curve(scores.class0 = na.omit(tcga_glm_elasticnet[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A" & factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis==(c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i],i]), scores.class1 = na.omit(tcga_glm_elasticnet[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A" & factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!=(c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i],i]), curve = TRUE )})
plot_pr_classify_glm_elasticnet = bind_rows(lapply(1:6, function(i){curve = temp_pr_classify_glm_elasticnet[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i])}))
temp_pr_classify_rf=lapply(1:6, function(i){pr.curve(scores.class0 = na.omit(tcga_rf[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A" & factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis==(c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i],i]), scores.class1 = na.omit(tcga_rf[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A" & factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!=(c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i],i]), curve = TRUE )})
plot_pr_classify_rf = bind_rows(lapply(1:6, function(i){curve = temp_pr_classify_rf[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i])}))
temp_pr_classify_xgb=lapply(1:6, function(i){pr.curve(scores.class0 = na.omit(tcga_xgb[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A" & factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis==(c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i],i]), scores.class1 = na.omit(tcga_xgb[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A" & factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!=(c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i],i]), curve = TRUE )})
plot_pr_classify_xgb = bind_rows(lapply(1:6, function(i){curve = temp_pr_classify_xgb[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i])}))

fig1a_1=ggplot(plot_pr_classify_glm_lasso, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (Lasso GLM)") + xlab("Recall (Lasso GLM)") + labs(color="Glioma Type") + scale_color_manual(labels = c(paste0("A (G2) (AUPRC=",signif(temp_pr_classify_glm_lasso[[1]]$auc.davis.goadrich,2),")"),paste0("A (G3) (AUPRC=",signif(temp_pr_classify_glm_lasso[[2]]$auc.davis.goadrich,2),")"),paste0("A (G4) (AUPRC=",signif(temp_pr_classify_glm_lasso[[3]]$auc.davis.goadrich,2),")"),paste0("GBM (G4) (AUPRC=",signif(temp_pr_classify_glm_lasso[[4]]$auc.davis.goadrich,2),")"),paste0("O (G2) (AUPRC=",signif(temp_pr_classify_glm_lasso[[5]]$auc.davis.goadrich,2),")"),paste0("O (G3) (AUPRC=",signif(temp_pr_classify_glm_lasso[[6]]$auc.davis.goadrich,2),")")), values = c("yellow2","orange2","firebrick2","purple2","lightblue2","dodgerblue2")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA))
fig1a_2=ggplot(plot_pr_classify_glm_ridge, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (Ridge GLM)") + xlab("Recall (Ridge GLM)") + labs(color="Glioma Type") + scale_color_manual(labels = c(paste0("A (G2) (AUPRC=",signif(temp_pr_classify_glm_ridge[[1]]$auc.davis.goadrich,2),")"),paste0("A (G3) (AUPRC=",signif(temp_pr_classify_glm_ridge[[2]]$auc.davis.goadrich,2),")"),paste0("A (G4) (AUPRC=",signif(temp_pr_classify_glm_ridge[[3]]$auc.davis.goadrich,2),")"),paste0("GBM (G4) (AUPRC=",signif(temp_pr_classify_glm_ridge[[4]]$auc.davis.goadrich,2),")"),paste0("O (G2) (AUPRC=",signif(temp_pr_classify_glm_ridge[[5]]$auc.davis.goadrich,2),")"),paste0("O (G3) (AUPRC=",signif(temp_pr_classify_glm_ridge[[6]]$auc.davis.goadrich,2),")")), values = c("yellow2","orange2","firebrick2","purple2","lightblue2","dodgerblue2")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA))
fig1a_3=ggplot(plot_pr_classify_glm_elasticnet, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (Elastic Net GLM)") + xlab("Recall (Elastic Net GLM)") + labs(color="Glioma Type") + scale_color_manual(labels = c(paste0("A (G2) (AUPRC=",signif(temp_pr_classify_glm_elasticnet[[1]]$auc.davis.goadrich,2),")"),paste0("A (G3) (AUPRC=",signif(temp_pr_classify_glm_elasticnet[[2]]$auc.davis.goadrich,2),")"),paste0("A (G4) (AUPRC=",signif(temp_pr_classify_glm_elasticnet[[3]]$auc.davis.goadrich,2),")"),paste0("GBM (G4) (AUPRC=",signif(temp_pr_classify_glm_elasticnet[[4]]$auc.davis.goadrich,2),")"),paste0("O (G2) (AUPRC=",signif(temp_pr_classify_glm_elasticnet[[5]]$auc.davis.goadrich,2),")"),paste0("O (G3) (AUPRC=",signif(temp_pr_classify_glm_elasticnet[[6]]$auc.davis.goadrich,2),")")), values = c("yellow2","orange2","firebrick2","purple2","lightblue2","dodgerblue2")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA))
fig1a_4=ggplot(plot_pr_classify_rf, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (Random Forest)") + xlab("Recall (Random Forest)") + labs(color="Glioma Type") + scale_color_manual(labels = c(paste0("A (G2) (AUPRC=",signif(temp_pr_classify_rf[[1]]$auc.davis.goadrich,2),")"),paste0("A (G3) (AUPRC=",signif(temp_pr_classify_rf[[2]]$auc.davis.goadrich,2),")"),paste0("A (G4) (AUPRC=",signif(temp_pr_classify_rf[[3]]$auc.davis.goadrich,2),")"),paste0("GBM (G4) (AUPRC=",signif(temp_pr_classify_rf[[4]]$auc.davis.goadrich,2),")"),paste0("O (G2) (AUPRC=",signif(temp_pr_classify_rf[[5]]$auc.davis.goadrich,2),")"),paste0("O (G3) (AUPRC=",signif(temp_pr_classify_rf[[6]]$auc.davis.goadrich,2),")")), values = c("yellow2","orange2","firebrick2","purple2","lightblue2","dodgerblue2")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA))
fig1a_5=ggplot(plot_pr_classify_xgb, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (XGBoost)") + xlab("Recall (XGBoost)") + labs(color="Glioma Type") + scale_color_manual(labels = c(paste0("A (G2) (AUPRC=",signif(temp_pr_classify_xgb[[1]]$auc.davis.goadrich,2),")"),paste0("A (G3) (AUPRC=",signif(temp_pr_classify_xgb[[2]]$auc.davis.goadrich,2),")"),paste0("A (G4) (AUPRC=",signif(temp_pr_classify_xgb[[3]]$auc.davis.goadrich,2),")"),paste0("GBM (G4) (AUPRC=",signif(temp_pr_classify_xgb[[4]]$auc.davis.goadrich,2),")"),paste0("O (G2) (AUPRC=",signif(temp_pr_classify_xgb[[5]]$auc.davis.goadrich,2),")"),paste0("O (G3) (AUPRC=",signif(temp_pr_classify_xgb[[6]]$auc.davis.goadrich,2),")")), values = c("yellow2","orange2","firebrick2","purple2","lightblue2","dodgerblue2")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA))


temp_roc_classify_glm_lasso=lapply(1:6, function(i){roc(response = as.integer(factors_gbmlgg_tcga[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A",]$WHO_CNS5_2021_Diagnosis == (c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i]),predictor = tcga_glm_lasso[factors_gbmlgg_tcga$WHO_CNS5_2021_Grade!="N/A",i],levels = c(1,0))})
temp_roc_classify_glm_ridge=lapply(1:6, function(i){roc(response = as.integer(factors_gbmlgg_tcga[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A",]$WHO_CNS5_2021_Diagnosis == (c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i]),predictor = tcga_glm_ridge[factors_gbmlgg_tcga$WHO_CNS5_2021_Grade!="N/A",i],levels = c(1,0))})
temp_roc_classify_glm_elasticnet=lapply(1:6, function(i){roc(response = as.integer(factors_gbmlgg_tcga[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A",]$WHO_CNS5_2021_Diagnosis == (c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i]),predictor = tcga_glm_elasticnet[factors_gbmlgg_tcga$WHO_CNS5_2021_Grade!="N/A",i],levels = c(1,0))})
temp_roc_classify_rf=lapply(1:6, function(i){roc(response = as.integer(factors_gbmlgg_tcga[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A",]$WHO_CNS5_2021_Diagnosis == (c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i]),predictor = tcga_rf[factors_gbmlgg_tcga$WHO_CNS5_2021_Grade!="N/A",i],levels = c(1,0))})
temp_roc_classify_xgb=lapply(1:6, function(i){roc(response = as.integer(factors_gbmlgg_tcga[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A",]$WHO_CNS5_2021_Diagnosis == (c("Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Glioblastoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3"))[i]),predictor = tcga_xgb[factors_gbmlgg_tcga$WHO_CNS5_2021_Grade!="N/A",i],levels = c(1,0))})

figs1a_1=ggroc(temp_roc_classify_glm_lasso)+theme_classic()+ylab("Sensitivity (Lasso GLM)")+xlab("Specificity (Lasso GLM)")+labs(color = "Glioma Type")+scale_color_manual(labels = c(paste0("A (G2) (AUROC=",signif(temp_roc_classify_glm_lasso[[1]]$auc[1],2),")"),paste0("A (G3) (AUROC=",signif(temp_roc_classify_glm_lasso[[2]]$auc[1],2),")"),paste0("A (G4) (AUROC=",signif(temp_roc_classify_glm_lasso[[3]]$auc[1],2),")"),paste0("GBM (G4) (AUROC=",signif(temp_roc_classify_glm_lasso[[4]]$auc[1],2),")"),paste0("O (G2) (AUROC=",signif(temp_roc_classify_glm_lasso[[5]]$auc[1],2),")"),paste0("O (G3) (AUROC=",signif(temp_roc_classify_glm_lasso[[6]]$auc[1],2),")")), values = c("yellow2","orange2","firebrick2","purple2","lightblue2","dodgerblue2"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))
figs1a_2=ggroc(temp_roc_classify_glm_ridge)+theme_classic()+ylab("Sensitivity (Ridge GLM)")+xlab("Specificity (Ridge GLM)")+labs(color = "Glioma Type")+scale_color_manual(labels = c(paste0("A (G2) (AUROC=",signif(temp_roc_classify_glm_ridge[[1]]$auc[1],2),")"),paste0("A (G3) (AUROC=",signif(temp_roc_classify_glm_ridge[[2]]$auc[1],2),")"),paste0("A (G4) (AUROC=",signif(temp_roc_classify_glm_ridge[[3]]$auc[1],2),")"),paste0("GBM (G4) (AUROC=",signif(temp_roc_classify_glm_ridge[[4]]$auc[1],2),")"),paste0("O (G2) (AUROC=",signif(temp_roc_classify_glm_ridge[[5]]$auc[1],2),")"),paste0("O (G3) (AUROC=",signif(temp_roc_classify_glm_ridge[[6]]$auc[1],2),")")), values = c("yellow2","orange2","firebrick2","purple2","lightblue2","dodgerblue2"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))
figs1a_3=ggroc(temp_roc_classify_glm_elasticnet)+theme_classic()+ylab("Sensitivity (Elastic Net GLM)")+xlab("Specificity (Elastic Net GLM)")+labs(color = "Glioma Type")+scale_color_manual(labels = c(paste0("A (G2) (AUROC=",signif(temp_roc_classify_glm_elasticnet[[1]]$auc[1],2),")"),paste0("A (G3) (AUROC=",signif(temp_roc_classify_glm_elasticnet[[2]]$auc[1],2),")"),paste0("A (G4) (AUROC=",signif(temp_roc_classify_glm_elasticnet[[3]]$auc[1],2),")"),paste0("GBM (G4) (AUROC=",signif(temp_roc_classify_glm_elasticnet[[4]]$auc[1],2),")"),paste0("O (G2) (AUROC=",signif(temp_roc_classify_glm_elasticnet[[5]]$auc[1],2),")"),paste0("O (G3) (AUROC=",signif(temp_roc_classify_glm_elasticnet[[6]]$auc[1],2),")")), values = c("yellow2","orange2","firebrick2","purple2","lightblue2","dodgerblue2"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))
figs1a_4=ggroc(temp_roc_classify_rf)+theme_classic()+ylab("Sensitivity (Random Forest)")+xlab("Specificity (Random Forest)")+labs(color = "Glioma Type")+scale_color_manual(labels = c(paste0("A (G2) (AUROC=",signif(temp_roc_classify_rf[[1]]$auc[1],2),")"),paste0("A (G3) (AUROC=",signif(temp_roc_classify_rf[[2]]$auc[1],2),")"),paste0("A (G4) (AUROC=",signif(temp_roc_classify_rf[[3]]$auc[1],2),")"),paste0("GBM (G4) (AUROC=",signif(temp_roc_classify_rf[[4]]$auc[1],2),")"),paste0("O (G2) (AUROC=",signif(temp_roc_classify_rf[[5]]$auc[1],2),")"),paste0("O (G3) (AUROC=",signif(temp_roc_classify_rf[[6]]$auc[1],2),")")), values = c("yellow2","orange2","firebrick2","purple2","lightblue2","dodgerblue2"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))
figs1a_5=ggroc(temp_roc_classify_xgb)+theme_classic()+ylab("Sensitivity (XGBoost)")+xlab("Specificity (XGBoost)")+labs(color = "Glioma Type")+scale_color_manual(labels = c(paste0("A (G2) (AUROC=",signif(temp_roc_classify_xgb[[1]]$auc[1],2),")"),paste0("A (G3) (AUROC=",signif(temp_roc_classify_xgb[[2]]$auc[1],2),")"),paste0("A (G4) (AUROC=",signif(temp_roc_classify_xgb[[3]]$auc[1],2),")"),paste0("GBM (G4) (AUROC=",signif(temp_roc_classify_xgb[[4]]$auc[1],2),")"),paste0("O (G2) (AUROC=",signif(temp_roc_classify_xgb[[5]]$auc[1],2),")"),paste0("O (G3) (AUROC=",signif(temp_roc_classify_xgb[[6]]$auc[1],2),")")), values = c("yellow2","orange2","firebrick2","purple2","lightblue2","dodgerblue2"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))


temp_pr_idh_glm_lasso=list(pr.curve(scores.class0 = rowSums(na.omit(tcga_glm_lasso[factors_gbmlgg_tcga$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(tcga_glm_lasso[factors_gbmlgg_tcga$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(cgga_glm_lasso[factors_gbmlgg_cgga$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(cgga_glm_lasso[factors_gbmlgg_cgga$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(glass_glm_lasso[factors_gbmlgg_glass$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(glass_glm_lasso[factors_gbmlgg_glass$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE))
plot_pr_idh_glm_lasso = bind_rows(lapply(1:3, function(i){curve = temp_pr_idh_glm_lasso[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("TCGA", "CGGA", "GLASS"))[i])}))
temp_pr_idh_glm_ridge=list(pr.curve(scores.class0 = rowSums(na.omit(tcga_glm_ridge[factors_gbmlgg_tcga$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(tcga_glm_ridge[factors_gbmlgg_tcga$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(cgga_glm_ridge[factors_gbmlgg_cgga$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(cgga_glm_ridge[factors_gbmlgg_cgga$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(glass_glm_ridge[factors_gbmlgg_glass$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(glass_glm_ridge[factors_gbmlgg_glass$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE))
plot_pr_idh_glm_ridge = bind_rows(lapply(1:3, function(i){curve = temp_pr_idh_glm_ridge[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("TCGA", "CGGA", "GLASS"))[i])}))
temp_pr_idh_glm_elasticnet=list(pr.curve(scores.class0 = rowSums(na.omit(tcga_glm_elasticnet[factors_gbmlgg_tcga$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(tcga_glm_elasticnet[factors_gbmlgg_tcga$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(cgga_glm_elasticnet[factors_gbmlgg_cgga$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(cgga_glm_elasticnet[factors_gbmlgg_cgga$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(glass_glm_elasticnet[factors_gbmlgg_glass$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(glass_glm_elasticnet[factors_gbmlgg_glass$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE))
plot_pr_idh_glm_elasticnet = bind_rows(lapply(1:3, function(i){curve = temp_pr_idh_glm_elasticnet[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("TCGA", "CGGA", "GLASS"))[i])}))
temp_pr_idh_rf=list(pr.curve(scores.class0 = rowSums(na.omit(tcga_rf[factors_gbmlgg_tcga$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(tcga_rf[factors_gbmlgg_tcga$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(cgga_rf[factors_gbmlgg_cgga$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(cgga_rf[factors_gbmlgg_cgga$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(glass_rf[factors_gbmlgg_glass$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(glass_rf[factors_gbmlgg_glass$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE))
plot_pr_idh_rf = bind_rows(lapply(1:3, function(i){curve = temp_pr_idh_rf[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("TCGA", "CGGA", "GLASS"))[i])}))
temp_pr_idh_xgb=list(pr.curve(scores.class0 = rowSums(na.omit(tcga_xgb[factors_gbmlgg_tcga$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(tcga_xgb[factors_gbmlgg_tcga$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(cgga_xgb[factors_gbmlgg_cgga$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(cgga_xgb[factors_gbmlgg_cgga$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(glass_xgb[factors_gbmlgg_glass$IDH_Status=="Mutant",])[,c(-4)]), scores.class1 = rowSums(na.omit(glass_xgb[factors_gbmlgg_glass$IDH_Status!="Mutant",])[,c(-4)]), curve = TRUE))
plot_pr_idh_xgb = bind_rows(lapply(1:3, function(i){curve = temp_pr_idh_xgb[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("TCGA", "CGGA", "GLASS"))[i])}))

fig1b_1=ggplot(plot_pr_idh_glm_lasso, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (Lasso GLM)") + xlab("Recall (Lasso GLM)") + labs(color="Dataset") + scale_color_manual(labels = c(paste0("TCGA (AUPRC=",signif(temp_pr_idh_glm_lasso[[1]]$auc.davis.goadrich,2),")"),paste0("CGGA (AUPRC=",signif(temp_pr_idh_glm_lasso[[2]]$auc.davis.goadrich,2),")"),paste0("GLASS (AUPRC=",signif(temp_pr_idh_glm_lasso[[3]]$auc.davis.goadrich,2),")")), values = c("darkred","darkgreen","darkblue")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA)) 
fig1b_2=ggplot(plot_pr_idh_glm_ridge, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (Ridge GLM)") + xlab("Recall (Ridge GLM)") + labs(color="Dataset") + scale_color_manual(labels = c(paste0("TCGA (AUPRC=",signif(temp_pr_idh_glm_ridge[[1]]$auc.davis.goadrich,2),")"),paste0("CGGA (AUPRC=",signif(temp_pr_idh_glm_ridge[[2]]$auc.davis.goadrich,2),")"),paste0("GLASS (AUPRC=",signif(temp_pr_idh_glm_ridge[[3]]$auc.davis.goadrich,2),")")), values = c("darkred","darkgreen","darkblue")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA)) 
fig1b_3=ggplot(plot_pr_idh_glm_elasticnet, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (Elastic Net GLM)") + xlab("Recall (Elastic Net GLM)") + labs(color="Dataset") + scale_color_manual(labels = c(paste0("TCGA (AUPRC=",signif(temp_pr_idh_glm_elasticnet[[1]]$auc.davis.goadrich,2),")"),paste0("CGGA (AUPRC=",signif(temp_pr_idh_glm_elasticnet[[2]]$auc.davis.goadrich,2),")"),paste0("GLASS (AUPRC=",signif(temp_pr_idh_glm_elasticnet[[3]]$auc.davis.goadrich,2),")")), values = c("darkred","darkgreen","darkblue")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA)) 
fig1b_4=ggplot(plot_pr_idh_rf, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (Random Forest)") + xlab("Recall (Random Forest)") + labs(color="Dataset") + scale_color_manual(labels = c(paste0("TCGA (AUPRC=",signif(temp_pr_idh_rf[[1]]$auc.davis.goadrich,2),")"),paste0("CGGA (AUPRC=",signif(temp_pr_idh_rf[[2]]$auc.davis.goadrich,2),")"),paste0("GLASS (AUPRC=",signif(temp_pr_idh_rf[[3]]$auc.davis.goadrich,2),")")), values = c("darkred","darkgreen","darkblue")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA)) 
fig1b_5=ggplot(plot_pr_idh_xgb, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (XGBoost)") + xlab("Recall (XGBoost)") + labs(color="Dataset") + scale_color_manual(labels = c(paste0("TCGA (AUPRC=",signif(temp_pr_idh_xgb[[1]]$auc.davis.goadrich,2),")"),paste0("CGGA (AUPRC=",signif(temp_pr_idh_xgb[[2]]$auc.davis.goadrich,2),")"),paste0("GLASS (AUPRC=",signif(temp_pr_idh_xgb[[3]]$auc.davis.goadrich,2),")")), values = c("darkred","darkgreen","darkblue")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA)) 


temp_roc_idh_glm_lasso=list(roc(sapply(factors_gbmlgg_tcga[factors_gbmlgg_tcga$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(tcga_glm_lasso[factors_gbmlgg_tcga$IDH_Status!="N/A",c(-4)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_cgga[factors_gbmlgg_cgga$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(cgga_glm_lasso[factors_gbmlgg_cgga$IDH_Status!="N/A",c(-4)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_glass[factors_gbmlgg_glass$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(glass_glm_lasso[factors_gbmlgg_glass$IDH_Status!="N/A",c(-4)]),levels = c(1,0)))
temp_roc_idh_glm_ridge=list(roc(sapply(factors_gbmlgg_tcga[factors_gbmlgg_tcga$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(tcga_glm_ridge[factors_gbmlgg_tcga$IDH_Status!="N/A",c(-4)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_cgga[factors_gbmlgg_cgga$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(cgga_glm_ridge[factors_gbmlgg_cgga$IDH_Status!="N/A",c(-4)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_glass[factors_gbmlgg_glass$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(glass_glm_ridge[factors_gbmlgg_glass$IDH_Status!="N/A",c(-4)]),levels = c(1,0)))
temp_roc_idh_glm_elasticnet=list(roc(sapply(factors_gbmlgg_tcga[factors_gbmlgg_tcga$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(tcga_glm_elasticnet[factors_gbmlgg_tcga$IDH_Status!="N/A",c(-4)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_cgga[factors_gbmlgg_cgga$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(cgga_glm_elasticnet[factors_gbmlgg_cgga$IDH_Status!="N/A",c(-4)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_glass[factors_gbmlgg_glass$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(glass_glm_elasticnet[factors_gbmlgg_glass$IDH_Status!="N/A",c(-4)]),levels = c(1,0)))
temp_roc_idh_rf=list(roc(sapply(factors_gbmlgg_tcga[factors_gbmlgg_tcga$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(tcga_rf[factors_gbmlgg_tcga$IDH_Status!="N/A",c(-4)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_cgga[factors_gbmlgg_cgga$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(cgga_rf[factors_gbmlgg_cgga$IDH_Status!="N/A",c(-4)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_glass[factors_gbmlgg_glass$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(glass_rf[factors_gbmlgg_glass$IDH_Status!="N/A",c(-4)]),levels = c(1,0)))
temp_roc_idh_xgb=list(roc(sapply(factors_gbmlgg_tcga[factors_gbmlgg_tcga$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(tcga_xgb[factors_gbmlgg_tcga$IDH_Status!="N/A",c(-4)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_cgga[factors_gbmlgg_cgga$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(cgga_xgb[factors_gbmlgg_cgga$IDH_Status!="N/A",c(-4)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_glass[factors_gbmlgg_glass$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),predictor = rowSums(glass_xgb[factors_gbmlgg_glass$IDH_Status!="N/A",c(-4)]),levels = c(1,0)))

figs1b_1=ggroc(temp_roc_idh_glm_lasso)+theme_classic()+ylab("Sensitivity (Lasso GLM)")+xlab("Specificity (Lasso GLM)")+labs(color = "Dataset")+scale_color_manual(labels = c(paste0("TCGA (AUROC=",signif(temp_roc_idh_glm_lasso[[1]]$auc[1],2),")"),paste0("CGGA (AUROC=",signif(temp_roc_idh_glm_lasso[[2]]$auc[1],2),")"),paste0("GLASS (AUROC=",signif(temp_roc_idh_glm_lasso[[3]]$auc[1],2),")")), values = c("darkred","darkgreen","darkblue"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))
figs1b_2=ggroc(temp_roc_idh_glm_ridge)+theme_classic()+ylab("Sensitivity (Ridge GLM)")+xlab("Specificity (Ridge GLM)")+labs(color = "Dataset")+scale_color_manual(labels = c(paste0("TCGA (AUROC=",signif(temp_roc_idh_glm_ridge[[1]]$auc[1],2),")"),paste0("CGGA (AUROC=",signif(temp_roc_idh_glm_ridge[[2]]$auc[1],2),")"),paste0("GLASS (AUROC=",signif(temp_roc_idh_glm_ridge[[3]]$auc[1],2),")")), values = c("darkred","darkgreen","darkblue"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))
figs1b_3=ggroc(temp_roc_idh_glm_elasticnet)+theme_classic()+ylab("Sensitivity (Elastic Net GLM)")+xlab("Specificity (Elastic Net GLM)")+labs(color = "Dataset")+scale_color_manual(labels = c(paste0("TCGA (AUROC=",signif(temp_roc_idh_glm_elasticnet[[1]]$auc[1],2),")"),paste0("CGGA (AUROC=",signif(temp_roc_idh_glm_elasticnet[[2]]$auc[1],2),")"),paste0("GLASS (AUROC=",signif(temp_roc_idh_glm_elasticnet[[3]]$auc[1],2),")")), values = c("darkred","darkgreen","darkblue"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))
figs1b_4=ggroc(temp_roc_idh_rf)+theme_classic()+ylab("Sensitivity (Random Forest)")+xlab("Specificity (Random Forest)")+labs(color = "Dataset")+scale_color_manual(labels = c(paste0("TCGA (AUROC=",signif(temp_roc_idh_rf[[1]]$auc[1],2),")"),paste0("CGGA (AUROC=",signif(temp_roc_idh_rf[[2]]$auc[1],2),")"),paste0("GLASS (AUROC=",signif(temp_roc_idh_rf[[3]]$auc[1],2),")")), values = c("darkred","darkgreen","darkblue"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))
figs1b_5=ggroc(temp_roc_idh_xgb)+theme_classic()+ylab("Sensitivity (XGBoost)")+xlab("Specificity (XGBoost)")+labs(color = "Dataset")+scale_color_manual(labels = c(paste0("TCGA (AUROC=",signif(temp_roc_idh_xgb[[1]]$auc[1],2),")"),paste0("CGGA (AUROC=",signif(temp_roc_idh_xgb[[2]]$auc[1],2),")"),paste0("GLASS (AUROC=",signif(temp_roc_idh_xgb[[3]]$auc[1],2),")")), values = c("darkred","darkgreen","darkblue"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))


temp_pr_codel_glm_lasso=list(pr.curve(scores.class0 = rowSums(na.omit(tcga_glm_lasso[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(tcga_glm_lasso[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(cgga_glm_lasso[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(cgga_glm_lasso[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(glass_glm_lasso[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(glass_glm_lasso[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE))
plot_pr_codel_glm_lasso = bind_rows(lapply(1:3, function(i){curve = temp_pr_codel_glm_lasso[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("TCGA", "CGGA", "GLASS"))[i])}))
temp_pr_codel_glm_ridge=list(pr.curve(scores.class0 = rowSums(na.omit(tcga_glm_ridge[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(tcga_glm_ridge[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(cgga_glm_ridge[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(cgga_glm_ridge[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(glass_glm_ridge[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(glass_glm_ridge[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE))
plot_pr_codel_glm_ridge = bind_rows(lapply(1:3, function(i){curve = temp_pr_codel_glm_ridge[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("TCGA", "CGGA", "GLASS"))[i])}))
temp_pr_codel_glm_elasticnet=list(pr.curve(scores.class0 = rowSums(na.omit(tcga_glm_elasticnet[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(tcga_glm_elasticnet[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(cgga_glm_elasticnet[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(cgga_glm_elasticnet[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(glass_glm_elasticnet[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(glass_glm_elasticnet[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE))
plot_pr_codel_glm_elasticnet = bind_rows(lapply(1:3, function(i){curve = temp_pr_codel_glm_elasticnet[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("TCGA", "CGGA", "GLASS"))[i])}))
temp_pr_codel_rf=list(pr.curve(scores.class0 = rowSums(na.omit(tcga_rf[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(tcga_rf[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(cgga_rf[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(cgga_rf[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(glass_rf[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(glass_rf[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE))
plot_pr_codel_rf = bind_rows(lapply(1:3, function(i){curve = temp_pr_codel_rf[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("TCGA", "CGGA", "GLASS"))[i])}))
temp_pr_codel_xgb=list(pr.curve(scores.class0 = rowSums(na.omit(tcga_xgb[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(tcga_xgb[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(cgga_xgb[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(cgga_xgb[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE), pr.curve(scores.class0 = rowSums(na.omit(glass_xgb[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion=="Codeleted",])[,-c(1:4)]), scores.class1 = rowSums(na.omit(glass_xgb[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="Codeleted",])[,-c(1:4)]), curve = TRUE))
plot_pr_codel_xgb = bind_rows(lapply(1:3, function(i){curve = temp_pr_codel_xgb[[i]]$curve; data.frame(Recall = curve[,1], Precision = curve[,2], Class = (c("TCGA", "CGGA", "GLASS"))[i])}))

fig1c_1=ggplot(plot_pr_codel_glm_lasso, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (Lasso GLM)") + xlab("Recall (Lasso GLM)") + labs(color="Dataset") + scale_color_manual(labels = c(paste0("TCGA (AUPRC=",signif(temp_pr_codel_glm_lasso[[1]]$auc.davis.goadrich,2),")"),paste0("CGGA (AUPRC=",signif(temp_pr_codel_glm_lasso[[2]]$auc.davis.goadrich,2),")"),paste0("GLASS (AUPRC=",signif(temp_pr_codel_glm_lasso[[3]]$auc.davis.goadrich,2),")")), values = c("darkred","darkgreen","darkblue")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA)) 
fig1c_2=ggplot(plot_pr_codel_glm_ridge, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (Ridge GLM)") + xlab("Recall (Ridge GLM)") + labs(color="Dataset") + scale_color_manual(labels = c(paste0("TCGA (AUPRC=",signif(temp_pr_codel_glm_ridge[[1]]$auc.davis.goadrich,2),")"),paste0("CGGA (AUPRC=",signif(temp_pr_codel_glm_ridge[[2]]$auc.davis.goadrich,2),")"),paste0("GLASS (AUPRC=",signif(temp_pr_codel_glm_ridge[[3]]$auc.davis.goadrich,2),")")), values = c("darkred","darkgreen","darkblue")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA)) 
fig1c_3=ggplot(plot_pr_codel_glm_elasticnet, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (Elastic Net GLM)") + xlab("Recall (Elastic Net GLM)") + labs(color="Dataset") + scale_color_manual(labels = c(paste0("TCGA (AUPRC=",signif(temp_pr_codel_glm_elasticnet[[1]]$auc.davis.goadrich,2),")"),paste0("CGGA (AUPRC=",signif(temp_pr_codel_glm_elasticnet[[2]]$auc.davis.goadrich,2),")"),paste0("GLASS (AUPRC=",signif(temp_pr_codel_glm_elasticnet[[3]]$auc.davis.goadrich,2),")")), values = c("darkred","darkgreen","darkblue")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA)) 
fig1c_4=ggplot(plot_pr_codel_rf, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (Random Forest)") + xlab("Recall (Random Forest)") + labs(color="Dataset") + scale_color_manual(labels = c(paste0("TCGA (AUPRC=",signif(temp_pr_codel_rf[[1]]$auc.davis.goadrich,2),")"),paste0("CGGA (AUPRC=",signif(temp_pr_codel_rf[[2]]$auc.davis.goadrich,2),")"),paste0("GLASS (AUPRC=",signif(temp_pr_codel_rf[[3]]$auc.davis.goadrich,2),")")), values = c("darkred","darkgreen","darkblue")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA)) 
fig1c_5=ggplot(plot_pr_codel_xgb, aes(x=Recall, y=Precision, color=factor(Class,levels=unique(Class)))) + geom_line(size=0.5) + theme_classic() + ylab("Precision (XGBoost)") + xlab("Recall (XGBoost)") + labs(color="Dataset") + scale_color_manual(labels = c(paste0("TCGA (AUPRC=",signif(temp_pr_codel_xgb[[1]]$auc.davis.goadrich,2),")"),paste0("CGGA (AUPRC=",signif(temp_pr_codel_xgb[[2]]$auc.davis.goadrich,2),")"),paste0("GLASS (AUPRC=",signif(temp_pr_codel_xgb[[3]]$auc.davis.goadrich,2),")")), values = c("darkred","darkgreen","darkblue")) + theme(legend.position = c(0.425, 0.325), text = element_text(size=6), legend.key.height = unit(0.5, "lines"), legend.background = element_rect(fill = NA, colour = NA)) 


temp_roc_codel_glm_lasso=list(roc(sapply(factors_gbmlgg_tcga[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(tcga_glm_lasso[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_cgga[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(cgga_glm_lasso[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_glass[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(glass_glm_lasso[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)))
temp_roc_codel_glm_ridge=list(roc(sapply(factors_gbmlgg_tcga[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(tcga_glm_ridge[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_cgga[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(cgga_glm_ridge[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_glass[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(glass_glm_ridge[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)))
temp_roc_codel_glm_elasticnet=list(roc(sapply(factors_gbmlgg_tcga[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(tcga_glm_elasticnet[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_cgga[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(cgga_glm_elasticnet[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_glass[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(glass_glm_elasticnet[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)))
temp_roc_codel_rf=list(roc(sapply(factors_gbmlgg_tcga[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(tcga_rf[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_cgga[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(cgga_rf[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_glass[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(glass_rf[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)))
temp_roc_codel_xgb=list(roc(sapply(factors_gbmlgg_tcga[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(tcga_xgb[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_cgga[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(cgga_xgb[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)),roc(sapply(factors_gbmlgg_glass[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion, switch,"Codeleted"=1,"Non_Codeleted"=0),predictor = rowSums(glass_xgb[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="N/A",c(-5,-6)]),levels = c(1,0)))

figs1c_1=ggroc(temp_roc_codel_glm_lasso)+theme_classic()+ylab("Sensitivity (Lasso GLM)")+xlab("Specificity (Lasso GLM)")+labs(color = "Dataset")+scale_color_manual(labels = c(paste0("TCGA (AUROC=",signif(temp_roc_codel_glm_lasso[[1]]$auc[1],2),")"),paste0("CGGA (AUROC=",signif(temp_roc_codel_glm_lasso[[2]]$auc[1],2),")"),paste0("GLASS (AUROC=",signif(temp_roc_codel_glm_lasso[[3]]$auc[1],2),")")), values = c("darkred","darkgreen","darkblue"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))
figs1c_2=ggroc(temp_roc_codel_glm_ridge)+theme_classic()+ylab("Sensitivity (Ridge GLM)")+xlab("Specificity (Ridge GLM)")+labs(color = "Dataset")+scale_color_manual(labels = c(paste0("TCGA (AUROC=",signif(temp_roc_codel_glm_ridge[[1]]$auc[1],2),")"),paste0("CGGA (AUROC=",signif(temp_roc_codel_glm_ridge[[2]]$auc[1],2),")"),paste0("GLASS (AUROC=",signif(temp_roc_codel_glm_ridge[[3]]$auc[1],2),")")), values = c("darkred","darkgreen","darkblue"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))
figs1c_3=ggroc(temp_roc_codel_glm_elasticnet)+theme_classic()+ylab("Sensitivity (Elastic Net GLM)")+xlab("Specificity (Elastic Net GLM)")+labs(color = "Dataset")+scale_color_manual(labels = c(paste0("TCGA (AUROC=",signif(temp_roc_codel_glm_elasticnet[[1]]$auc[1],2),")"),paste0("CGGA (AUROC=",signif(temp_roc_codel_glm_elasticnet[[2]]$auc[1],2),")"),paste0("GLASS (AUROC=",signif(temp_roc_codel_glm_elasticnet[[3]]$auc[1],2),")")), values = c("darkred","darkgreen","darkblue"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))
figs1c_4=ggroc(temp_roc_codel_rf)+theme_classic()+ylab("Sensitivity (Random Forest)")+xlab("Specificity (Random Forest)")+labs(color = "Dataset")+scale_color_manual(labels = c(paste0("TCGA (AUROC=",signif(temp_roc_codel_rf[[1]]$auc[1],2),")"),paste0("CGGA (AUROC=",signif(temp_roc_codel_rf[[2]]$auc[1],2),")"),paste0("GLASS (AUROC=",signif(temp_roc_codel_rf[[3]]$auc[1],2),")")), values = c("darkred","darkgreen","darkblue"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))
figs1c_5=ggroc(temp_roc_codel_xgb)+theme_classic()+ylab("Sensitivity (XGBoost)")+xlab("Specificity (XGBoost)")+labs(color = "Dataset")+scale_color_manual(labels = c(paste0("TCGA (AUROC=",signif(temp_roc_codel_xgb[[1]]$auc[1],2),")"),paste0("CGGA (AUROC=",signif(temp_roc_codel_xgb[[2]]$auc[1],2),")"),paste0("GLASS (AUROC=",signif(temp_roc_codel_xgb[[3]]$auc[1],2),")")), values = c("darkred","darkgreen","darkblue"))+theme(legend.position = c(0.525, 0.425))+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.background = element_rect(fill = NA, colour = NA))


factors_gbmlgg_tcga$Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis=apply(tcga_glm_elasticnet, 1, function(x) { colnames(tcga_glm_elasticnet)[which.max(x)] })
factors_gbmlgg_cgga$Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis=apply(cgga_glm_elasticnet, 1, function(x) { colnames(cgga_glm_elasticnet)[which.max(x)] })
factors_gbmlgg_glass$Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis=apply(glass_glm_elasticnet, 1, function(x) { colnames(glass_glm_elasticnet)[which.max(x)] })
...
brier_per_class = function(y_true, y_prob) {
  y_true = factor(y_true)
  classes = levels(y_true)
  sapply(classes, function(cls) {
    y_bin = ifelse(y_true == cls, 1, 0)
    mean((y_bin - y_prob[[cls]])^2)
  })
}

temp=data.frame(Glioma_Type=c("A (G2)","A (G3)","A (G4)","GBM (G4)","O (G2)","O (G3)"),Brier_Score=brier_per_class(factors_gbmlgg_tcga[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A",]$WHO_CNS5_2021_Diagnosis,tcga_glm_elasticnet[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A",]))
figs2a=ggplot(temp, aes(x=Glioma_Type, y=Brier_Score, fill=Glioma_Type))+geom_bar(stat = "identity")+theme_classic()+scale_fill_manual(values=c(`GBM (G4)`="purple2",`A (G2)`="yellow2",`A (G3)`="orange2",`A (G4)`="firebrick2",`O (G2)`="lightblue2",`O (G3)`="dodgerblue2"))+xlab("Glioma Type")+ylab("Brier Score")+ labs(fill = "Glioma Type")+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.key.width = unit(0.5, "lines"),legend.position = "top",legend.justification = "center",legend.direction = "horizontal")+ scale_y_continuous(breaks=seq(0,0.15,by=0.01))+guides(fill=guide_legend(title="Glioma\nType", nrow = 2, byrow = TRUE))

temp=data.frame(Glioma_Type=c("A (G2)","A (G3)","A (G4)","GBM (G4)","O (G2)","O (G3)"),F1=confusionMatrix(data=factor(factors_gbmlgg_tcga[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A",]$Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis,levels=c("Glioblastoma_G4","Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3")),reference=factor(factors_gbmlgg_tcga[factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis!="N/A",]$WHO_CNS5_2021_Diagnosis,levels=c("Glioblastoma_G4","Astrocytoma_G2","Astrocytoma_G3","Astrocytoma_G4","Oligodendroglioma_G2","Oligodendroglioma_G3")))$byClass[,7])
figs2b=ggplot(temp, aes(x=Glioma_Type, y=F1, fill=Glioma_Type))+geom_bar(stat = "identity")+theme_classic()+scale_fill_manual(values=c(`GBM (G4)`="purple2",`A (G2)`="yellow2",`A (G3)`="orange2",`A (G4)`="firebrick2",`O (G2)`="lightblue2",`O (G3)`="dodgerblue2"))+xlab("Glioma Type")+ylab("F1")+ labs(fill = "Glioma Type")+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.key.width = unit(0.5, "lines"),legend.position = "top",legend.justification = "center",legend.direction = "horizontal")+ scale_y_continuous(breaks=seq(0,1,by=0.1))+guides(fill=guide_legend(title="Glioma\nType", nrow = 2, byrow = TRUE))

temp=data.frame(Dataset=c("TCGA","CGGA","GLASS"), Brier_Score=c(brier_per_class(factors_gbmlgg_tcga[factors_gbmlgg_tcga$IDH_Status!="N/A",]$IDH_Status,data.frame(Mutant=rowSums(tcga_glm_elasticnet[factors_gbmlgg_tcga$IDH_Status!="N/A",c(-4)]),Wildtype=tcga_glm_elasticnet[factors_gbmlgg_tcga$IDH_Status!="N/A",c(4)]))[1],brier_per_class(factors_gbmlgg_cgga[factors_gbmlgg_cgga$IDH_Status!="N/A",]$IDH_Status,data.frame(Mutant=rowSums(cgga_glm_elasticnet[factors_gbmlgg_cgga$IDH_Status!="N/A",c(-4)]),Wildtype=cgga_glm_elasticnet[factors_gbmlgg_cgga$IDH_Status!="N/A",c(4)]))[1],brier_per_class(factors_gbmlgg_glass[factors_gbmlgg_glass$IDH_Status!="N/A",]$IDH_Status,data.frame(Mutant=rowSums(glass_glm_elasticnet[factors_gbmlgg_glass$IDH_Status!="N/A",c(-4)]),Wildtype=glass_glm_elasticnet[factors_gbmlgg_glass$IDH_Status!="N/A",c(4)]))[1]))
figs2c=ggplot(temp, aes(x=Dataset, y=Brier_Score, fill=Dataset))+geom_bar(stat = "identity")+theme_classic()+scale_fill_manual(values=c(`TCGA`="darkred", `CGGA`="darkgreen", `GLASS`="darkblue"))+xlab("Dataset")+ylab("Brier Score\n(IDH Status)")+ labs(fill = "Dataset")+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.key.width = unit(0.5, "lines"),legend.position = "top",legend.justification = "center",legend.direction = "horizontal")+ scale_y_continuous(breaks=seq(0,0.15,by=0.01))

temp=data.frame(Dataset=c("TCGA","CGGA","GLASS"),F1=c(confusionMatrix(data=factor(sapply(factors_gbmlgg_tcga[factors_gbmlgg_tcga$IDH_Status!="N/A",]$Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis, switch,"Astrocytoma_G2"=1,"Astrocytoma_G3"=1,"Astrocytoma_G4"=1,"Glioblastoma_G4"=0,"Oligodendroglioma_G2"=1,"Oligodendroglioma_G3"=1),levels = c(0,1)),reference=factor(sapply(factors_gbmlgg_tcga[factors_gbmlgg_tcga$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),levels = c(0,1)))$byClass[7], confusionMatrix(data=factor(sapply(factors_gbmlgg_cgga[factors_gbmlgg_cgga$IDH_Status!="N/A",]$Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis, switch,"Astrocytoma_G2"=1,"Astrocytoma_G3"=1,"Astrocytoma_G4"=1,"Glioblastoma_G4"=0,"Oligodendroglioma_G2"=1,"Oligodendroglioma_G3"=1),levels = c(0,1)),reference=factor(sapply(factors_gbmlgg_cgga[factors_gbmlgg_cgga$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),levels = c(0,1)))$byClass[7], confusionMatrix(data=factor(sapply(factors_gbmlgg_glass[factors_gbmlgg_glass$IDH_Status!="N/A",]$Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis, switch,"Astrocytoma_G2"=1,"Astrocytoma_G3"=1,"Astrocytoma_G4"=1,"Glioblastoma_G4"=0,"Oligodendroglioma_G2"=1,"Oligodendroglioma_G3"=1),levels = c(0,1)),reference=factor(sapply(factors_gbmlgg_glass[factors_gbmlgg_glass$IDH_Status!="N/A",]$IDH_Status, switch,"Mutant"=1,"Wildtype"=0),levels = c(0,1)))$byClass[7]))
figs2d=ggplot(temp, aes(x=Dataset, y=F1, fill=Dataset))+geom_bar(stat = "identity")+theme_classic()+scale_fill_manual(values=c(`TCGA`="darkred", `CGGA`="darkgreen", `GLASS`="darkblue"))+xlab("Dataset")+ylab("F1\n(IDH Status)")+ labs(fill = "Dataset")+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.key.width = unit(0.5, "lines"),legend.position = "top",legend.justification = "center",legend.direction = "horizontal")+ scale_y_continuous(breaks=seq(0,1,by=0.1))

temp=data.frame(Dataset=c("TCGA","CGGA","GLASS"), Brier_Score=c(brier_per_class(factors_gbmlgg_tcga[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion,data.frame(Non_Codeleted=rowSums(tcga_glm_elasticnet[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="N/A",-c(5,6)]),Codeleted=rowSums(tcga_glm_elasticnet[factors_gbmlgg_tcga$Chromosome_1p_19q_Codeletion!="N/A",c(5,6)])))[1],brier_per_class(factors_gbmlgg_cgga[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion,data.frame(Non_Codeleted=rowSums(cgga_glm_elasticnet[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="N/A",-c(5,6)]),Codeleted=rowSums(cgga_glm_elasticnet[factors_gbmlgg_cgga$Chromosome_1p_19q_Codeletion!="N/A",c(5,6)])))[1],brier_per_class(factors_gbmlgg_glass[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="N/A",]$Chromosome_1p_19q_Codeletion,data.frame(Non_Codeleted=rowSums(glass_glm_elasticnet[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="N/A",-c(5,6)]),Codeleted=rowSums(glass_glm_elasticnet[factors_gbmlgg_glass$Chromosome_1p_19q_Codeletion!="N/A",c(5,6)])))[1]))
figs2e=ggplot(temp, aes(x=Dataset, y=Brier_Score, fill=Dataset))+geom_bar(stat = "identity")+theme_classic()+scale_fill_manual(values=c(`TCGA`="darkred", `CGGA`="darkgreen", `GLASS`="darkblue"))+xlab("Dataset")+ylab("Brier Score\n(Chromosome 1p/19q-codeletion)")+ labs(fill = "Dataset")+theme(text = element_text(size = 6),legend.key.height = unit(0.5, "lines"),legend.key.width = unit(0.5, "lines"),legend.position = "top",legend.justification = "center",legend.direction = "horizontal")+ scale_y_continuous(breaks=seq(0,0.15,by=0.01))
...
temp_data=rbind(data.frame(Sample_ID=rownames(factors_gbmlgg_tcga),Dataset="TCGA",tcga_glm_elasticnet,Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis=factors_gbmlgg_tcga$Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis,Dropped_Sample=factors_gbmlgg_tcga$Dropped),
      data.frame(Sample_ID=rownames(factors_gbmlgg_cgga),Dataset="CGGA",cgga_glm_elasticnet,Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis=factors_gbmlgg_cgga$Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis,Dropped_Sample=factors_gbmlgg_cgga$Dropped),
      data.frame(Sample_ID=rownames(factors_gbmlgg_glass),Dataset="GLASS",glass_glm_elasticnet,Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis=factors_gbmlgg_glass$Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis,Dropped_Sample=factors_gbmlgg_glass$Dropped))
...
temp[]=lapply(temp, function(x) c("Glioblastoma_G4" = "GBM (G4)", "Astrocytoma_G2" = "A (G2)", "Astrocytoma_G3" = "A (G3)", "Astrocytoma_G4" = "A (G4)", "Oligodendroglioma_G2" = "O (G2)", "Oligodendroglioma_G3" = "O (G3)", "N/A" = "Unlabelled")[as.character(x)])
...
fig2a=ggplot(tcga_sankey, aes(x = x, next_x = next_x, node = node, next_node = next_node, fill = factor(node), label = node)) + geom_sankey(flow.alpha = .6, node.color = "black") + geom_sankey_label(size = 1.5, color = "white", fill = "black") + scale_fill_viridis_d(drop = FALSE) + theme_sankey() + labs(x = NULL) + theme(legend.position = "none", plot.title = element_text(hjust = .5))+scale_fill_manual(values=c(`Unlabelled`="black",`Misclassified`="black",`GBM (G4)`="purple2",`A (G2)`="yellow2",`A (G3)`="orange2",`A (G4)`="firebrick2",`O (G2)`="lightblue2",`O (G3)`="dodgerblue2"))+theme(text = element_text(size = 6),plot.margin = margin(0.1, -1.5, 0.1, -1.5, "lines"))


temp=factors_gbmlgg_cgga[,c("CGGA_Diagnosis","Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis")]
colnames(temp)=c("CGGA\nAnnotation","Predicted\nWHO CNS5 Annotation")
temp[]=lapply(temp, function(x) c("Glioblastoma_G4" = "GBM (G4)", "Glioblastoma_IV" = "GBM (IV)", "Astrocytoma_G2" = "A (G2)", "Astrocytoma_G3" = "A (G3)", "Astrocytoma_G4" = "A (G4)", "Astrocytoma_II" = "A (II)", "Astrocytoma_III" = "A (III)", "Astrocytoma_IV" = "A (IV)", "Oligodendroglioma_G2" = "O (G2)", "Oligodendroglioma_G3" = "O (G3)", "Oligodendroglioma_II" = "O (II)", "Oligodendroglioma_III" = "O (III)", "Anaplastic_Astrocytoma_III" = "AA (III)", "Anaplastic_Oligoastrocytoma_III" = "AOA (III)", "Anaplastic_Oligodendroglioma_III" = "AO (III)", "Oligoastrocytoma_II" = "OA (II)", "N/A" = "Unlabelled")[as.character(x)])
...
fig2d=ggplot(cgga_sankey, aes(x = x, next_x = next_x, node = node, next_node = next_node, fill = factor(node), label = node)) + geom_sankey(flow.alpha = .6, node.color = "black") + geom_sankey_label(size = 1.5, color = "white", fill = "black") + scale_fill_viridis_d(drop = FALSE) + theme_sankey() + labs(x = NULL) + theme(legend.position = "none", plot.title = element_text(hjust = .5))+scale_fill_manual(values=c(`Unlabelled`="black",`Misclassified`="black",`GBM (G4)`="purple2",`A (G2)`="yellow2",`A (G3)`="orange2",`A (G4)`="firebrick2",`O (G2)`="lightblue2",`O (G3)`="dodgerblue2",`GBM (IV)`="purple2",`A (II)`="yellow2",`A (III)`="orange2",`A (IV)`="firebrick2",`O (II)`="lightblue2",`O (III)`="dodgerblue2",`OA (II)`="palegreen2",`OA (III)`="seagreen2",`AA (III)`="orange1",`AA (II)`="yellow1",`AOA (II)`="palegreen1",`AOA (III)`="seagreen1",`AO (III)`="dodgerblue1"))+theme(text = element_text(size = 6),plot.margin = margin(0.1, -1.5, 0.1, -1.5, "lines"))


temp=factors_gbmlgg_glass[,c("GLASS_Diagnosis","Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis")]
colnames(temp)=c("GLASS\nAnnotation","Predicted\nWHO CNS5 Annotation")
temp[]=lapply(temp, function(x) c("Glioblastoma_G4" = "GBM (G4)", "Glioblastoma_IV" = "GBM (IV)", "Astrocytoma_G2" = "A (G2)", "Astrocytoma_G3" = "A (G3)", "Astrocytoma_G4" = "A (G4)", "Astrocytoma_II" = "A (II)", "Astrocytoma_III" = "A (III)", "Astrocytoma_IV" = "A (IV)", "Oligodendroglioma_G2" = "O (G2)", "Oligodendroglioma_G3" = "O (G3)", "Oligodendroglioma_II" = "O (II)", "Oligodendroglioma_III" = "O (III)", "Anaplastic_Astrocytoma_II" = "AA (II)", "Anaplastic_Astrocytoma_III" = "AA (III)", "Oligoastrocytoma_II" = "OA (II)", "Anaplastic_Oligoastrocytoma_II" = "AOA (II)", "Anaplastic_Oligoastrocytoma_III" = "AOA (III)", "Anaplastic_Oligodendroglioma_III" = "AO (III)", "Diffuse_Astrocytoma_II" = "DA (II)", "N/A" = "Unlabelled")[as.character(x)])
...
fig2g=ggplot(glass_sankey, aes(x = x, next_x = next_x, node = node, next_node = next_node, fill = factor(node), label = node)) + geom_sankey(flow.alpha = .6, node.color = "black") + geom_sankey_label(size = 1.5, color = "white", fill = "black") + scale_fill_viridis_d(drop = FALSE) + theme_sankey() + labs(x = NULL) + theme(legend.position = "none", plot.title = element_text(hjust = .5))+scale_fill_manual(values=c(`Misclassified`="black",`GBM (G4)`="purple2",`A (G2)`="yellow2",`A (G3)`="orange2",`A (G4)`="firebrick2",`O (G2)`="lightblue2",`O (G3)`="dodgerblue2",`GBM (IV)`="purple2",`A (II)`="yellow2",`A (III)`="orange2",`A (IV)`="firebrick2",`O (II)`="lightblue2",`O (III)`="dodgerblue2",`OA (II)`="palegreen2",`OA (III)`="seagreen2",`AA (III)`="orange1",`AA (II)`="yellow1",`AOA (II)`="palegreen1",`AOA (III)`="seagreen1",`AO (III)`="dodgerblue1",`DA (II)`="yellow3"))+theme(text = element_text(size = 6),plot.margin = margin(0.1, -1.5, 0.1, -1.5, "lines"))

##################################################################
#####UMAP (Bulk Glioma)#####
##################################################################
umap_tcga_gbmlgg=umap(t(cve_gene_gbmlgg_tcga[limma_filter(x,y,100),]))
df_umap = data.frame(x = umap_tcga_gbmlgg$layout[,1], y = umap_tcga_gbmlgg$layout[,2], color1=factors_gbmlgg_tcga$WHO_CNS5_2021_Diagnosis, color2=factors_gbmlgg_tcga$Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis, Dropped=factors_gbmlgg_tcga$Dropped_Sample)
df_umap[,c(3,4)]=lapply(df_umap[,c(3,4)], function(x) c("Glioblastoma_G4" = "GBM (G4)", "Astrocytoma_G2" = "A (G2)", "Astrocytoma_G3" = "A (G3)", "Astrocytoma_G4" = "A (G4)", "Oligodendroglioma_G2" = "O (G2)", "Oligodendroglioma_G3" = "O (G3)", "N/A" = "Unlabelled")[as.character(x)])
...
fig2b=ggplot(df_umap, aes(x, y, colour = color1)) + geom_point(size=1)+theme_classic()+scale_color_discrete()+scale_color_manual(values=c(`Unlabelled`="black",`GBM (G4)`="purple2",`A (G2)`="yellow2",`A (G3)`="orange2",`A (G4)`="firebrick2",`O (G2)`="lightblue2",`O (G3)`="dodgerblue2"))+guides(color=guide_legend(title="WHO CNS5\nAnnotation", nrow = 3, byrow = TRUE))+xlab("UMAP1")+ylab("UMAP2")+theme(text = element_text(size = 6),legend.position = "top",legend.justification = "center",legend.key.height = unit(0.25, "lines"),legend.key.width = unit(0.1, "lines"),legend.margin=margin(c(0,25,0,0)))
fig2c=ggplot(df_umap, aes(x, y, colour = color2)) + geom_point(size=1)+theme_classic()+scale_color_discrete()+scale_color_manual(values=c(`Misclassified`="black",`GBM (G4)`="purple2",`A (G2)`="yellow2",`A (G3)`="orange2",`A (G4)`="firebrick2",`O (G2)`="lightblue2",`O (G3)`="dodgerblue2"))+guides(color=guide_legend(title="Predicted TCGA\nWHO CNS5\nAnnotation", nrow = 3, byrow = TRUE))+xlab("UMAP1")+ylab("UMAP2")+theme(text = element_text(size = 6),legend.position = "top",legend.justification = "center",legend.key.height = unit(0.25, "lines"),legend.key.width = unit(0.1, "lines"),legend.margin=margin(c(0,25,0,0)))

umap_cgga_gbmlgg=umap(t(cve_gene_gbmlgg_cgga[limma_filter(x,y,100),]))
...
df_umap[,c(3,4)]=lapply(df_umap[,c(3,4)], function(x) c("Glioblastoma_G4" = "GBM (G4)", "Glioblastoma_IV" = "GBM (IV)", "Astrocytoma_G2" = "A (G2)", "Astrocytoma_G3" = "A (G3)", "Astrocytoma_G4" = "A (G4)", "Astrocytoma_II" = "A (II)", "Astrocytoma_III" = "A (III)", "Astrocytoma_IV" = "A (IV)", "Oligodendroglioma_G2" = "O (G2)", "Oligodendroglioma_G3" = "O (G3)", "Oligodendroglioma_II" = "O (II)", "Oligodendroglioma_III" = "O (III)", "Anaplastic_Astrocytoma_III" = "AA (III)", "Anaplastic_Oligoastrocytoma_III" = "AOA (III)", "Anaplastic_Oligodendroglioma_III" = "AO (III)", "Oligoastrocytoma_II" = "OA (II)", "N/A" = "Unlabelled")[as.character(x)])
df_umap[df_umap$Dropped==T,]$color2="Misclassified"
...
fig2e=ggplot(df_umap, aes(x, y, colour = color1)) + geom_point(size=1)+theme_classic()+scale_color_discrete()+scale_color_manual(values=c(`Unlabelled`="black",`Misclassified`="black",`GBM (G4)`="purple2",`A (G2)`="yellow2",`A (G3)`="orange2",`A (G4)`="firebrick2",`O (G2)`="lightblue2",`O (G3)`="dodgerblue2",`GBM (IV)`="purple2",`A (II)`="yellow2",`A (III)`="orange2",`A (IV)`="firebrick2",`O (II)`="lightblue2",`O (III)`="dodgerblue2",`OA (II)`="palegreen2",`OA (III)`="seagreen2",`AA (III)`="orange1",`AA (II)`="yellow1",`AOA (II)`="palegreen1",`AOA (III)`="seagreen1",`AO (III)`="dodgerblue1"))+guides(color=guide_legend(title="CGGA\nAnnotation", nrow = 3, byrow = TRUE))+xlab("UMAP1")+ylab("UMAP2")+theme(text = element_text(size = 6),legend.position = "top",legend.justification = "center",legend.key.height = unit(0.25, "lines"),legend.key.width = unit(0.1, "lines"),legend.margin=margin(c(0,25,0,0)))
fig2f=ggplot(df_umap, aes(x, y, colour = color2)) + geom_point(size=1)+theme_classic()+scale_color_discrete()+scale_color_manual(values=c(`Unlabelled`="black",`Misclassified`="black",`GBM (G4)`="purple2",`A (G2)`="yellow2",`A (G3)`="orange2",`A (G4)`="firebrick2",`O (G2)`="lightblue2",`O (G3)`="dodgerblue2",`GBM (IV)`="purple2",`A (II)`="yellow2",`A (III)`="orange2",`A (IV)`="firebrick2",`O (II)`="lightblue2",`O (III)`="dodgerblue2",`OA (II)`="palegreen2",`OA (III)`="seagreen2",`AA (III)`="orange1",`AA (II)`="yellow1",`AOA (II)`="palegreen1",`AOA (III)`="seagreen1",`AO (III)`="dodgerblue1"))+guides(color=guide_legend(title="Predicted CGGA\nWHO CNS5\nAnnotation", nrow = 3, byrow = TRUE))+xlab("UMAP1")+ylab("UMAP2")+theme(text = element_text(size = 6),legend.position = "top",legend.justification = "center",legend.key.height = unit(0.25, "lines"),legend.key.width = unit(0.1, "lines"),legend.margin=margin(c(0,25,0,0)))

umap_glass_gbmlgg=umap(t(cve_gene_gbmlgg_glass[limma_filter(x,y,100),]))
...
fig2h=ggplot(df_umap, aes(x, y, colour = color1)) + geom_point(size=1)+theme_classic()+scale_color_discrete()+scale_color_manual(values=c(`Misclassified`="black",`GBM (G4)`="purple2",`A (G2)`="yellow2",`A (G3)`="orange2",`A (G4)`="firebrick2",`O (G2)`="lightblue2",`O (G3)`="dodgerblue2",`GBM (IV)`="purple2",`A (II)`="yellow2",`A (III)`="orange2",`A (IV)`="firebrick2",`O (II)`="lightblue2",`O (III)`="dodgerblue2",`OA (II)`="palegreen2",`OA (III)`="seagreen2",`AA (III)`="orange1",`AA (II)`="yellow1",`AOA (II)`="palegreen1",`AOA (III)`="seagreen1",`AO (III)`="dodgerblue1",`DA (II)`="yellow3"))+guides(color=guide_legend(title="GLASS\nAnnotation", nrow = 3, byrow = TRUE))+xlab("UMAP1")+ylab("UMAP2")+theme(text = element_text(size = 6),legend.position = "top",legend.justification = "center",legend.key.height = unit(0.25, "lines"),legend.key.width = unit(0.1, "lines"),legend.margin=margin(c(0,25,0,0)))
fig2i=ggplot(df_umap, aes(x, y, colour = color2)) + geom_point(size=1)+theme_classic()+scale_color_discrete()+scale_color_manual(values=c(`Misclassified`="black",`GBM (G4)`="purple2",`A (G2)`="yellow2",`A (G3)`="orange2",`A (G4)`="firebrick2",`O (G2)`="lightblue2",`O (G3)`="dodgerblue2",`GBM (IV)`="purple2",`A (II)`="yellow2",`A (III)`="orange2",`A (IV)`="firebrick2",`O (II)`="lightblue2",`O (III)`="dodgerblue2",`OA (II)`="palegreen2",`OA (III)`="seagreen2",`AA (III)`="orange1",`AA (II)`="yellow1",`AOA (II)`="palegreen1",`AOA (III)`="seagreen1",`AO (III)`="dodgerblue1",`DA (II)`="yellow3"))+guides(color=guide_legend(title="Predicted GLASS\nWHO CNS5\nAnnotation", nrow = 3, byrow = TRUE))+xlab("UMAP1")+ylab("UMAP2")+theme(text = element_text(size = 6),legend.position = "top",legend.justification = "center",legend.key.height = unit(0.25, "lines"),legend.key.width = unit(0.1, "lines"),legend.margin=margin(c(0,25,0,0)))
...
temp[,c(2)]=sapply(temp[,c(2)], function(x) c("Glioblastoma_G4" = "GBM (G4)", "Astrocytoma_G2" = "A (G2)", "Astrocytoma_G3" = "A (G3)", "Astrocytoma_G4" = "A (G4)", "Oligodendroglioma_G2" = "O (G2)", "Oligodendroglioma_G3" = "O (G3)", "N/A" = "Unlabelled")[as.character(x)])
...
fig3a=(ggsurvplot(fit1,legend.title = "Glioma Type",surv.median.line = "hv",pval = TRUE, break.time.by = 12,risk.table = T, conf.int = F,palette = c("purple2","yellow2","orange2","firebrick2","lightblue2","dodgerblue2"),legend.labs=c("GBM (G4)", "A (G2)", "A (G3)", "A (G4)", "O (G2)", "O (G3)"),pval.coord = c(120, 0.95),test.for.trend = T,xlim=c(0,168),pval.size = 3,ggtheme = theme_classic(),size=0.5,censor.size=3))[[1]]+theme(text = element_text(size = 6),legend.key.height = unit(0.25, "lines"),legend.position = "top",legend.justification = "center")+xlab("Time (Months)")+ylab("Probability of Survival")
...
temp[,c(2,3)]=lapply(temp[,c(2,3)], function(x) c("Glioblastoma_G4" = "GBM (G4)", "Astrocytoma_G2" = "A (G2)", "Astrocytoma_G3" = "A (G3)", "Astrocytoma_G4" = "A (G4)", "Oligodendroglioma_G2" = "O (G2)", "Oligodendroglioma_G3" = "O (G3)", "N/A" = "Unlabelled")[as.character(x)])
temp[temp$Dropped_Sample==T,]$Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis="Misclassified"
...
figs3a=(ggsurvplot(fit1,legend.title = "WHO CNS5\nAnnotation",surv.median.line = "hv",pval = TRUE, break.time.by = 12,risk.table = T, conf.int = F,palette = c("purple2","yellow2","orange2","firebrick2","lightblue2","dodgerblue2"),legend.labs=c("GBM (G4)", "A (G2)", "A (G3)", "A (G4)", "O (G2)", "O (G3)"),pval.coord = c(120, 0.95),test.for.trend = T,xlim=c(0,168),pval.size = 3,ggtheme = theme_classic(),size=0.5,censor.size=3))[[1]]+theme(text = element_text(size = 6),legend.key.height = unit(0.25, "lines"),legend.position = "top",legend.justification = "center")+xlab("Time (Months)")+ylab("Probability of Survival")
...
temp[,c(2,3)]=lapply(temp[,c(2,3)], function(x) c("Glioblastoma_G4" = "GBM (G4)", "Glioblastoma_IV" = "GBM (IV)", "Astrocytoma_G2" = "A (G2)", "Astrocytoma_G3" = "A (G3)", "Astrocytoma_G4" = "A (G4)", "Astrocytoma_II" = "A (II)", "Astrocytoma_III" = "A (III)", "Astrocytoma_IV" = "A (IV)", "Oligodendroglioma_G2" = "O (G2)", "Oligodendroglioma_G3" = "O (G3)", "Oligodendroglioma_II" = "O (II)", "Oligodendroglioma_III" = "O (III)", "Anaplastic_Astrocytoma_III" = "AA (III)", "Anaplastic_Oligoastrocytoma_III" = "AOA (III)", "Anaplastic_Oligodendroglioma_III" = "AO (III)", "Oligoastrocytoma_II" = "OA (II)", "N/A" = "Unlabelled")[as.character(x)])
temp[temp$Dropped_Sample==T,]$Predicted_GLM_ElasticNet_WHO_CNS5_2021_Diagnosis="Misclassified"
...
figs3c=(ggsurvplot(fit1,legend.title = "CGGA\nAnnotation",surv.median.line = "hv",pval = TRUE, break.time.by = 12,risk.table = T, conf.int = F,palette = c("purple2","yellow2","orange1","palegreen2","seagreen1","lightblue2","dodgerblue1"),legend.labs=c("GBM (IV)", "A (II)", "AA (III)", "OA (II)", "AOA (II)", "O (II)", "AO (III)"),pval.coord = c(120, 0.95),test.for.trend = T,xlim=c(0,168),pval.size = 3,ggtheme = theme_classic(),size=0.5,censor.size=3))[[1]]+theme(text = element_text(size = 6),legend.key.height = unit(0.25, "lines"),legend.position = "top",legend.justification = "center")+xlab("Time (Months)")+ylab("Probability of Survival")
...
efit = eBayes(mmfit)
res_gene_glass_gbmlgg_pred_AstroII=data.frame(topTable(efit, coef="AstroII", sort.by = "none", n = Inf,lfc = 0))
...
colnames(res_gene_glass_gbmlgg_pred_AstroII)=c("log2FC","log2Expression","t_Statistic","pval","padj","logOdds_B")
...
x_astroii=data.frame(t(data.frame(res_gene_tcga_gbmlgg_og_AstroII$padj<0.05,res_gene_tcga_gbmlgg_pred_AstroII$padj<0.05,res_gene_cgga_gbmlgg_pred_AstroII$padj<0.05,res_gene_glass_gbmlgg_pred_AstroII$padj<0.05)))
colnames(x_astroii)=rownames(sub_dge_gene_gbmlgg_cgga)
rownames(x_astroii)=c(paste0("Original TCGA: ",paste0(table(res_gene_tcga_gbmlgg_og_AstroII$padj<0.05)[2]), " DEGs"), paste0("Predicted TCGA: ",paste0(table(res_gene_tcga_gbmlgg_pred_AstroII$padj<0.05)[2]), " DEGs"), paste0("Predicted CGGA: ",paste0(table(res_gene_cgga_gbmlgg_pred_AstroII$padj<0.05)[2]), " DEGs"), paste0("Predicted GLASS: ",paste0(table(res_gene_glass_gbmlgg_pred_AstroII$padj<0.05)[2]), " DEGs"))
...
fig4f=plotEnrich(enrich_gbmlgg_OligoIII, showTerms = 14, numChar = 100,xlab="Enriched Terms",ylab="Gene Count",orderBy = "P.value")+theme_classic()+ facet_grid(.~Sign)+scale_fill_continuous(high="dodgerblue4",low="dodgerblue1",labels = scales::label_number(accuracy = 1))+ theme(text = element_text(size = 6), plot.title = element_blank(),legend.position = "bottom",legend.justification = "left",legend.key.width  = unit(0.75, "lines"),legend.key.height = unit(0.4, "lines"),legend.margin=margin(c(0,0,0,0)))+ guides(fill=guide_colourbar(title="Log10 Adjusted\np-value"))
...
write.csv(factors_gbmlgg_tcga2[c(1,28:29,31)],"Pub_Classify/TCGA_Glioma_ESTIMATE_Deconvolution.csv",row.names = F)
...
#####Final Figure Order#####
##################################################################
plot1_1=plot_grid(fig1a_1, fig1b_1, fig1c_1, labels = c('A', 'B', 'C'), label_size = 10, rel_widths = c(1, 1, 1),ncol = 3)
plot1_2=plot_grid(fig1a_2, fig1b_2, fig1c_2, labels = c('', '', ''), label_size = 10, rel_widths = c(1, 1, 1),ncol = 3)
plot1_3=plot_grid(fig1a_3, fig1b_3, fig1c_3, labels = c('', '', ''), label_size = 10, rel_widths = c(1, 1, 1),ncol = 3)
plot1_4=plot_grid(fig1a_4, fig1b_4, fig1c_4, labels = c('', '', ''), label_size = 10, rel_widths = c(1, 1, 1),ncol = 3)
plot1_5=plot_grid(fig1a_5, fig1b_5, fig1c_5, labels = c('', '', ''), label_size = 10, rel_widths = c(1, 1, 1),ncol = 3)

pdf("Pub_Classify/Fig1.pdf",height=9,width=6.5)
plot_grid("",plot1_1,plot1_2,plot1_3,plot1_4,plot1_5, ncol = 1,rel_heights=c(0.1,1,1,1,1,1))
dev.off()

plots1_1=plot_grid(figs1a_1, figs1b_1, figs1c_1, labels = c('A', 'B', 'C'), label_size = 10, rel_widths = c(1, 1, 1),ncol = 3)
plots1_2=plot_grid(figs1a_2, figs1b_2, figs1c_2, labels = c('', '', ''), label_size = 10, rel_widths = c(1, 1, 1),ncol = 3)
plots1_3=plot_grid(figs1a_3, figs1b_3, figs1c_3, labels = c('', '', ''), label_size = 10, rel_widths = c(1, 1, 1),ncol = 3)
plots1_4=plot_grid(figs1a_4, figs1b_4, figs1c_4, labels = c('', '', ''), label_size = 10, rel_widths = c(1, 1, 1),ncol = 3)
plots1_5=plot_grid(figs1a_5, figs1b_5, figs1c_5, labels = c('', '', ''), label_size = 10, rel_widths = c(1, 1, 1),ncol = 3)

pdf("Pub_Classify/SupFig1.pdf",height=9,width=6.5)
plot_grid("",plots1_1,plots1_2,plots1_3,plots1_4,plots1_5, ncol = 1,rel_heights=c(0.1,1,1,1,1,1))
dev.off()
...
plot_grid("",plot2_1,plot2_2,plot2_3, ncol = 1,rel_heights=c(0.1*8.5/9*4/6,1,1,1))
dev.off()
...
plot_grid("",plots3_1,plots3_2, plots3_3, ncol = 1,rel_heights=c(0.1*7/9*4/6,1,1,1))
dev.off()
...
plot_grid("",plot4_1,plot4_2,plot4_3, ncol = 1,rel_heights=c(0.1*4/6,1,1,1))
dev.off()
...
plot_grid("",plots4_1,plots4_2,plots4_3, ncol = 1,rel_heights=c(0.1*6/9*4/6,1,1,1))
dev.off()
...
plot_grid("",plot5_1,plot5_2,plot5_3, ncol = 1,rel_heights=c(0.1*8/9*4/6,1.5,1,1))
dev.off()
...
plot_grid("",plots5_1,plots5_2, plots5_3, ncol = 1,rel_heights=c(0.1*8/9*4/6,1,1,1))
dev.off()
